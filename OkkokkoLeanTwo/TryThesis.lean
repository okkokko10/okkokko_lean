...
-- example : LinearEquiv (@Rn n) (Fin n → ℝ) := by sorry
...
def Λ.minimum_distance (norm : Rn → ℝ≥0) := ⨅ (x ∈ (Λ B)) (_ : x ≠ 0), norm x
...
-/
-- i or more
def Λ.successive_minimum_distance (norm : (@Rn n) → ℝ≥0) (i : ℕ)
  := ⨅ (r : ℝ≥0) (_ : ∃s ⊆ ((Λ B).carrier), LinearIndependent ℝ (Subtype.val : s → Rn) ∧ s.encard ≤ i ∧ ∀x ∈ s, norm x ≤ r), r
...
--
-- about paper: "A lattice is a discrete additive subgroup of ℝⁿ",
...
#check DiscreteTopology -- Λ has this?
    -- 𝓛 is written \MCL
